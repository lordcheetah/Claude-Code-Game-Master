...
**Record the beat:** when scene context reports `Record narration: ON` (default), after you persist and narrate a beat, save its **narrative prose** (the story as told — NOT the status bar or the numbered menu) with `bash tools/gm-narrate.sh record "<prose>" [--image <plate.png>]`. This is the spine of the chronicle ebook. (Player toggles with `gm-session.sh record on|off`.)
...
| Narration beat (chronicle ebook) | `gm-narrate.sh record "<prose>" [--image <plate.png>]` — the story as told, sans status bar/menu; when `Record narration: ON` |
...
- **Chronicle / ebook:** `bash tools/gm-chronicle.sh [campaign]` compiles a played campaign into an illustrated **EPUB** via pandoc. Its spine is the **recorded narration** (`gm-narrate.sh` beats in `narration-log.jsonl`) — the actual played prose, one chapter per session, with the chronicler's image **plates dropped in where they were generated** (matched by timestamp), plus the character dossier + NPC roster. Sessions with no recorded prose (played before recording, or with it toggled off) fall back to their session-log summary, so old campaigns still compile. Flags: `--cover N` (Nth plate as cover), `--no-appendix`, `--md-only`. Deterministic — it arranges recorded material, it doesn't invent prose.
