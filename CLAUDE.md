...
- **Multiplayer (online):** `bash tools/gm-relay.sh serve --lan --code <word>` starts a thin, self-hosted browser relay so remote players can join a multi-PC table (needs `gm-session.sh multiplayer on` + seats via `gm-party.sh`). It never touches the GM's reasoning — players' typed actions queue in `relay/inbox.jsonl`; the GM `drain`s them each beat and `post`s narration (`--image` to attach a plate) to `relay/outbox.jsonl`, which the relay serves back. One shared session stays the whole GM; single-writer-per-file means no locking. See the `gm-multiplayer` skill for the beat loop.
