...
case "$ACTION" in
    ...
    record)
        $PYTHON_CMD "$LIB_DIR/session_manager.py" record "$@"
        ;;

    *)
        echo "Unknown action: $ACTION"
        echo "Valid actions: start, end, status, move, context, choices, multiplayer, record, save, restore, list-saves, delete-save, history"
        exit 1
        ;;
esac
